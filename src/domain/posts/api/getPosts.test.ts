...
describe("getPosts", () => {
  ...
  it("should handle project posts without cover image", async () => {
    ...
    expect((posts[0] as any).coverImage).toBeUndefined();
  });
  ...
  it("should handle project posts with file cover", async () => {
    ...
    expect((posts[0] as any).coverImage).toBe("https://example.com/file.jpg");
  });
  ...
  it("should handle book posts", async () => {
    ...
    expect(posts).toHaveLength(1);
    expect((posts[0] as any).author).toBeDefined();
    expect((posts[0] as any).rating).toBeDefined();
  });
